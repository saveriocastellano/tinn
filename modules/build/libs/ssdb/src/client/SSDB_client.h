...
public:
	 ...
	 */
	bool error(){
		return code_ != "ok" && code_ != "not_found";
	}
	/**
	 * The response code.
	 */
...
public:
	...
	/// @{
	//virtual void pipelineStart() = 0;
	//virtual bool pipelinedRequest(const std::vector<std::string> &req) = 0;
	//virtual bool pipelineExec() = 0;
	virtual bool  pipelinedCommands(const std::vector<std::vector<std::string>> &req) = 0;
	...
	virtual Status getset(const std::string &key, const std::string &val, std::string *rval) = 0;
	...
	virtual Status zremrangebyscore(const std::string &name, int64_t scoreMin, int64_t scoreMax, int64_t *ret) = 0;
	virtual Status zcount(const std::string &name, int64_t min, int64_t max, int64_t *ret) = 0;
	...
	virtual Status qlist(const std::string &name, const std::string &start, const std::string &end, int64_t limit, std::vector<std::string> *ret) = 0; 
