...
credits: 
	- 80% of the code in this file was taken from d8.cc of google v8 engine
*/
...
typedef bool (*moduleAttachFunc)(v8::Isolate* isolate, v8::Local<v8::Context> &context);
...
std::string GetExePath() {
	...
	strcat(path,(char *)getauxval(AT_EXECFN));
	int lastSl = (int)lastIndexOf(path, "/");
	if (lastSl >=0) path[lastSl] = '\0';
...
{
#ifdef _WIN32
	std::string epath = GetExePath() + "\\modules";
	
	HANDLE hFind;
	WIN32_FIND_DATAA FindFileData;

	if((hFind = FindFirstFileA((epath+"\\*.dll").c_str(), &FindFileData)) != INVALID_HANDLE_VALUE){
		do{
			//printf("%s\n", FindFileData.cFileName);
			std::string lpath = epath + "\\" + FindFileData.cFileName;
			HMODULE hModule = LoadLibraryA(lpath.c_str());
			moduleAttachFunc maf = NULL;
			moduleInitFunc mif = NULL;
			
			if (hModule!=NULL){
				maf = (moduleAttachFunc)GetProcAddress((HMODULE)hModule, "attach");
				mif = (moduleInitFunc)GetProcAddress((HMODULE)hModule, "init");
				if (maf != NULL && mif != NULL){
				...
				printf("module %s failed to initialize (LoadLibrary failed, error code %lu)\n", lpath.c_str(), GetLastError());
			}
	...
	sprintf(modulesPath, "%s", epath.c_str()); 
...
typedef struct {
	std::map<unsigned int, FILE*> handles;
	int num;
} OSContext;
...
static void OSFileSize(const v8::FunctionCallbackInfo<v8::Value>& args) {
	...
	v8::String::Utf8Value jsName(isolate,Local<v8::String>::Cast(args[0]));
...
static void OSSetEnv(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  v8::String::Utf8Value jsName(isolate,Local<v8::String>::Cast(args[0]));
  v8::String::Utf8Value jsVal(isolate,Local<v8::String>::Cast(args[1]));
...
static void OSGetEnv(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (!val && args.Length() > 1 && args[1]->IsString())
  {
	...
	Local<v8::String> jsVal = v8::String::NewFromUtf8(isolate, val).ToLocalChecked();;
...
void OSExec(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	Local<Array> execArgs = Local<Array>::Cast(args[0]);

	std::stringstream ss;
	for (unsigned int i=0; i<execArgs->Length();i++)
	{
		v8::String::Utf8Value jsArg(isolate,Local<v8::String>::Cast(execArgs->Get(context, i).ToLocalChecked()));
    ...
    if (!pipe)
	{
		...
		if (close) {
			setres= inst->Set(context, v8::String::NewFromUtf8(isolate, "result").ToLocalChecked(), v8::Integer::New(isolate,pclose(pipe))).FromMaybe(0L);		
		} else {
			char buffer[128];
			...
			setres= inst->Set(context, v8::String::NewFromUtf8(isolate, "result").ToLocalChecked(), v8::Integer::New(isolate,pclose(pipe))).FromMaybe(0L);	
			setres= inst->Set(context, v8::String::NewFromUtf8(isolate, "output").ToLocalChecked(), v8::String::NewFromUtf8(isolate, res.str().c_str()).ToLocalChecked()).FromMaybe(0L);	
		}
	}
...
static void OSWriteFile(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	v8::String::Utf8Value jsName(isolate,Local<v8::String>::Cast(args[0]));
	v8::String::Utf8Value jsContent(isolate,Local<v8::String>::Cast(args[1]));
	char * buf = *jsContent;
...
static void OSListDir(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	v8::String::Utf8Value jsDir(isolate,Local<v8::String>::Cast(args[0]));
	...
	if (args.Length() > 1)
	{
		v8::String::Utf8Value jsExt(isolate,Local<v8::String>::Cast(args[1]));
		ext = strdup(*jsExt);
	}
	...
	while ((dirp = readdir(dp)) != NULL) {
		...
		if (ext && (lio == 0 || lio != (int)(strlen(dirp->d_name)-strlen(ext)))) continue;
		files.push_back(std::string(dirp->d_name));
    }
	Local<Array> results = v8::Array::New(isolate, (int)files.size());
	bool setres;
	for (int i=0; i<(int)files.size(); i++) {
		setres= results->Set(context, i, v8::String::NewFromUtf8(isolate, files[i].c_str()).ToLocalChecked()).FromMaybe(0L);
	}	
	if (ext) free(ext);
    closedir(dp);
...
static void OSLastModifiedTime(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	args.GetReturnValue().Set( v8::Integer::New(isolate, (int)statbuf.st_mtime));
}
...
static void OSFopen(const v8::FunctionCallbackInfo<v8::Value>&  args)
{
	...
	std::string file(*String::Utf8Value(isolate,args[0]));
	FILE * f = fopen(file.c_str(), (*String::Utf8Value(isolate,args[1])));
	if (!f)
	{
		 Throw(isolate, (std::string("Error opening file ") + file).c_str());
		 return;
	}
	ctx->num ++;
...
static void OSWritebytes(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	Local<Array> jsBytes = Local<Array>::Cast(args[1]);
	BYTE * buf = new BYTE[jsBytes->Length()];
	unsigned int count = (unsigned int)jsBytes->Length();
	for (unsigned int i=0; i<count;i++)
	{
		Local<v8::Value> b =  jsBytes->Get(context, i).ToLocalChecked();
		if (!b->IsUint32()) {
			delete[] buf;
	...
	int num = (int)fwrite(buf, 1, count, ctx->handles[fd]);
	delete[] buf;
...
static void OSReadbytes(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	int bytesRead = (int)fread(buf, 1, count, ctx->handles[fd]);
	Local<Array> jsBytes = v8::Array::New(isolate, bytesRead);
	bool setres;
	for (int i=0; i<bytesRead; i++)
	{
		setres =jsBytes->Set(context, i, v8::Integer::New(isolate, buf[i])).FromMaybe(0L);;
	}
	delete[] buf;
...
static void OSReadString(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	int bytesRead = (int)fread(buf, 1, count, ctx->handles[fd]);
	buf[bytesRead] = '\0';
	Local<v8::String> jsVal = v8::String::NewFromUtf8(isolate, buf).ToLocalChecked();
	args.GetReturnValue().Set( jsVal);
	delete[] buf;
}
...
static void OSWriteString(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	v8::String::Utf8Value jsString(isolate, Local<v8::String>::Cast(args[1]));	
...
static void OSGetHostname(const v8::FunctionCallbackInfo<v8::Value>& args)
{
	...
	Local<v8::String> jsVal = v8::String::NewFromUtf8(isolate, hostname).ToLocalChecked();
	args.GetReturnValue().Set(jsVal);	  
}
...
MaybeLocal<Context> Shell::CreateRealm(
  ...
  args.GetReturnValue().Set(index);
    ...
    //attach os..
	AddTINNOS(isolate, context);  

    //attach modules
    for (auto it : gModules)
    {
	  it->attach(isolate, context);
    }	  
...
void Shell::Run(const v8::FunctionCallbackInfo<v8::Value>& args) {
	Isolate* isolate = args.GetIsolate();
	HandleScope handle_scope(isolate);
	
	std::string file = "<unknown>";
	if (args.Length() == 0 || !args[0]->IsString()) {
	  Throw(args.GetIsolate(), "Invalid arguments");
	  return;
		...
		String::Utf8Value jsFile(args.GetIsolate(), args[1]);
		file = *jsFile;
	}
  ...
  ReportExceptions report_exceptions = kReportExceptions;
	ProcessMessageQueue process_message_queue = kNoProcessMessageQueue;
  MaybeLocal<Value> maybe_result;
  bool success = true;
  {
    PerIsolateData* data = PerIsolateData::Get(isolate);
    ...
    MaybeLocal<Script> maybe_script;
    Local<Context> context(isolate->GetCurrentContext());
    ScriptOrigin origin(String::NewFromUtf8(isolate, file.c_str(), NewStringType::kNormal).ToLocalChecked());
...
void Shell::AddTINNOS(Isolate* isolate, v8::Local<v8::Context> &context) {
	...
	os->Set(v8::String::NewFromUtf8(isolate, "gethostname").ToLocalChecked(), FunctionTemplate::New(isolate, OSGetHostname));
	os->Set(v8::String::NewFromUtf8(isolate, "cwd").ToLocalChecked(), FunctionTemplate::New(isolate, OSCWD));
	os->Set(v8::String::NewFromUtf8(isolate, "sleep").ToLocalChecked(), FunctionTemplate::New(isolate, OSSleep));
	os->Set(v8::String::NewFromUtf8(isolate, "getEnv").ToLocalChecked(), FunctionTemplate::New(isolate, OSGetEnv));
	os->Set(v8::String::NewFromUtf8(isolate, "setEnv").ToLocalChecked(), FunctionTemplate::New(isolate, OSSetEnv));
	os->Set(v8::String::NewFromUtf8(isolate, "exec").ToLocalChecked(), FunctionTemplate::New(isolate, OSExec));
	os->Set(v8::String::NewFromUtf8(isolate, "readFile").ToLocalChecked(), FunctionTemplate::New(isolate, OSReadFile));
	os->Set(v8::String::NewFromUtf8(isolate, "writeFile").ToLocalChecked(), FunctionTemplate::New(isolate, OSWriteFile));
	os->Set(v8::String::NewFromUtf8(isolate, "listDir").ToLocalChecked(), FunctionTemplate::New(isolate, OSListDir));
	os->Set(v8::String::NewFromUtf8(isolate, "isFileAndReadable").ToLocalChecked(), FunctionTemplate::New(isolate, OSFileAndReadable));
	os->Set(v8::String::NewFromUtf8(isolate, "isDirAndReadable").ToLocalChecked(), FunctionTemplate::New(isolate, OSDirAndReadable));
	os->Set(v8::String::NewFromUtf8(isolate, "lastModifiedTime").ToLocalChecked(), FunctionTemplate::New(isolate, OSLastModifiedTime));
	os->Set(v8::String::NewFromUtf8(isolate, "unlink").ToLocalChecked(), FunctionTemplate::New(isolate, OSUnlink));
	os->Set(v8::String::NewFromUtf8(isolate, "rmdir").ToLocalChecked(), FunctionTemplate::New(isolate, OSRmdir));
	os->Set(v8::String::NewFromUtf8(isolate, "fseek").ToLocalChecked(), FunctionTemplate::New(isolate, OSFseek));
	os->Set(v8::String::NewFromUtf8(isolate, "fopen").ToLocalChecked(), FunctionTemplate::New(isolate, OSFopen));
	os->Set(v8::String::NewFromUtf8(isolate, "fclose").ToLocalChecked(), FunctionTemplate::New(isolate, OSFclose));
	os->Set(v8::String::NewFromUtf8(isolate, "readBytes").ToLocalChecked(), FunctionTemplate::New(isolate, OSReadbytes));
	os->Set(v8::String::NewFromUtf8(isolate, "writeString").ToLocalChecked(), FunctionTemplate::New(isolate, OSWriteString));
	os->Set(v8::String::NewFromUtf8(isolate, "readString").ToLocalChecked(), FunctionTemplate::New(isolate, OSReadString));
	os->Set(v8::String::NewFromUtf8(isolate, "writeBytes").ToLocalChecked(), FunctionTemplate::New(isolate, OSWritebytes));
	...
	context->Global()->Set(context,v8::String::NewFromUtf8(isolate,"os").ToLocalChecked(), instance).FromJust();	
...
Local<ObjectTemplate> Shell::CreateGlobalTemplate(Isolate* isolate) {
  ...
  global_template->Set(
      String::NewFromUtf8(isolate, "run", NewStringType::kNormal)
          .ToLocalChecked(),
...
/*	  
  ...
  AddTINNOSMethods(isolate, os_templ);
  
 
	os_templ->SetInternalFieldCount(1);  
	
	v8::Local<v8::Object> instance = os_templ->NewInstance(context).ToLocalChecked();	
	OSContext *ctx = new OSContext();	
	ctx->num = 0;
	instance->SetAlignedPointerInInternalField(0, ctx);		
...
*/
...
#ifdef _WIN32  
  process->Set(v8::String::NewFromUtf8(isolate, "platform").ToLocalChecked(), v8::String::NewFromUtf8(isolate,"win32").ToLocalChecked());
#else
  ...
  for (int i = 0; i < size; ++i) {
    ...
    const long length = s ? s - var : strlen(var);
    //envarr->Set(i, name);
	char key[length+1];
	strncpy(key, var, length);
	key[length] ='\0';
	envs[std::string(key)] = std::string((char*)getenv(key));
	...
	while(it != envs.end()) {
		process_env_template->Set(String::NewFromUtf8(isolate, it->first.c_str(), NewStringType::kNormal).ToLocalChecked(), 
				String::NewFromUtf8(isolate,it->second.c_str(), NewStringType::kNormal).ToLocalChecked());
		it++;
	}
  
  process->Set(String::NewFromUtf8(isolate, "env", NewStringType::kNormal).ToLocalChecked(), process_env_template);

  

  
  global_template->Set(String::NewFromUtf8(isolate, "process", NewStringType::kNormal).ToLocalChecked(),  process);
...
void Shell::InitJS(Isolate *isolate) {	
	char initJSpath[FILENAME_MAX];
#if _WIN32	
	sprintf(initJSpath, "%s\\%s", GetExePath().c_str(), ".init.js");
#else	
	sprintf(initJSpath, "%s/%s", GetExePath().c_str(), ".init.js");
#endif	
	if (isFileReadable(initJSpath)){
		...
		if (!ExecuteString(isolate, str, String::NewFromUtf8(isolate, "(tinn)", NewStringType::kNormal).ToLocalChecked(),
				...
				options.quiet_load ? kNoReportExceptions : kReportExceptions,
				kNoProcessMessageQueue)) {
		  printf("Error loading .init.js file\n");
