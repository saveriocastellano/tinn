...
/*
int __pipelineRequestNum = 0;
void ClientImpl::pipelineStart()  {
	__pipelineRequestNum = 0;
}

bool ClientImpl::pipelinedRequest(const std::vector<std::string> &req){
	if(link->send(req) == -1){
		return false;
	}
	__pipelineRequestNum ++;
	return true;
}

bool ClientImpl::pipelineExec(){
	if(link->flush() == -1){
		return false;
	}
	while(__pipelineRequestNum > 0) {
		link->response();
		__pipelineRequestNum--;
	}
	return true;		
...
bool  ClientImpl::pipelinedCommands(const std::vector<std::vector<std::string>> &reqs){
	int count = 0;
	int numCmd = reqs.size();
...
Status ClientImpl::getset(const std::string &key, const std::string &val, std::string *rval){
	const std::vector<std::string> *resp;
	resp = this->request("getset", key, val);
	return _read_str(resp, rval);
}
...
Status ClientImpl::zremrangebyscore(const std::string &name, int64_t scoreMin, int64_t scoreMax, int64_t *ret){
	const std::vector<std::string> *resp;
	std::string s_scoreMin = str(scoreMin);
	std::string s_scoreMax = str(scoreMax);

	resp = this->request("zremrangebyscore", name, s_scoreMin, s_scoreMax);
	return _read_int64(resp, ret);
}
...
Status ClientImpl::zcount(const std::string &name, int64_t min, int64_t max, int64_t *ret)
{
	const std::vector<std::string> *resp;
	std::string s_scoreMin = str(min);
	std::string s_scoreMax = str(max);
	
	resp = this->request("zcount", name, s_scoreMin, s_scoreMax);
	return _read_int64(resp, ret);
}
...
Status ClientImpl::qlist(const std::string &name, const std::string &start, const std::string &end, int64_t limit, std::vector<std::string> *ret){
	const std::vector<std::string> *resp;
	std::string s_limit = str(limit);
	resp = this->request("qlist", name, start, end, s_limit);
	return _read_list(resp, ret);
}
